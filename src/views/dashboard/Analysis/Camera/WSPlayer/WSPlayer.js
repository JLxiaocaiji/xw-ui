...
class WSPlayer {
  ...
  resetPlayerScreen(allScreen, maxNum) {
    for (let i = 0; i < maxNum; i++) {
      allScreen[i] &&
        $(`#${allScreen[i].getAttribute("id")}`).css({
          top: `150%`,
          left: 0,
          width: 0,
          height: 0,
  ...
  __getWSUrl(rtspUrl, streamServerIp, wsList, playType) {
    return new Promise((resolve, reject) => {
      ...
      this.autoSetWSUrl(wsList, protocol)
        .then((wsUrl) => {
          resolve(wsUrl);
        })
        .catch((err) => {
          ...
          if (this.config.useNginxProxy) {
            ...
            let wssPort = this.type === "real" ? CONSTANT.websocketPorts.realmonitor : CONSTANT.websocketPorts.playback;
          ...
          if (protocol === "wss") {
            ...
            wsPort = this.type === "real" ? CONSTANT.websocketPorts.realmonitor_wss : CONSTANT.websocketPorts.playback_wss;
          } else {
            // http协议就用ws协议直连流媒体服务
            wsPort = this.type === "real" ? CONSTANT.websocketPorts.realmonitor_ws : CONSTANT.websocketPorts.playback_ws;
          }
